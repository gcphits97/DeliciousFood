...
public class AdapterFastFoodFragment extends RecyclerView.Adapter<AdapterFastFoodFragment.MyHolder> {
    ...
    public void onBindViewHolder(@NonNull final AdapterFastFoodFragment.MyHolder myHolder, int i) {
        ...
        DownloadImageBitmapFromURL(homeModel, myHolder.imageViewFastFood);
    ...
    private void DownloadImageBitmapFromURL(HomeModel homeModel, ImageView imageViewFastFood) {
        Picasso.get().load(homeModel.getHinhanhmonan())
                .placeholder(R.drawable.albedo)
                .error(R.mipmap.ic_error)
                .into(imageViewFastFood, new Callback() {
                    @Override
